...
from st_harmonic_training.tavern_structure import DOCUMENTED_WORK_IDS
...
class TavernLineageTests(unittest.TestCase):
    ...
    def test_alias_mapping_matches_central_exact_work_contract(self) -> None:
        _assert_mapping_contract()
        self.assertEqual(len(DOCUMENTED_WORK_IDS), 27)
    ...
    def test_count_preserving_work_substitution_fails_closed(self) -> None:
        structure = self.load_structure()
        structure["work_summaries"][0]["source_work_id"] = "Beethoven/Unexpected"
        with self.assertRaises(TavernLineageError):
            build_tavern_lineage_evidence(structure)
