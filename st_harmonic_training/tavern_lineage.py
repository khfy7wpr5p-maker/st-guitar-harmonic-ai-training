...
# These catalogue families define only the reviewed cross-corpus alias transform.
# The authoritative exact TAVERN work set is DOCUMENTED_WORK_IDS in tavern_structure.py.
...
def _mapping_source_work_ids() -> frozenset[str]:
    values = {f"Beethoven/B{number:03d}" for number in BEETHOVEN_WOO}
    values.update(f"Beethoven/Opus{number}" for number in BEETHOVEN_OPUS)
    values.update(f"Mozart/K{number:03d}" for number in MOZART_K)
    return frozenset(values)


def _assert_mapping_contract() -> None:
    mapping_ids = _mapping_source_work_ids()
    if mapping_ids != DOCUMENTED_WORK_IDS:
        missing = sorted(DOCUMENTED_WORK_IDS - mapping_ids)
        unexpected = sorted(mapping_ids - DOCUMENTED_WORK_IDS)
        raise TavernLineageError(
            "lineage alias mapping diverges from Stage 0-I documented work IDs; "
...
def build_tavern_lineage_evidence(structure_evidence: object) -> dict[str, object]:
    ...
    source_ids = sorted(DOCUMENTED_WORK_IDS)
    mappings = [_work_mapping(source_id) for source_id in source_ids]
